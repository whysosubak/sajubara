...
export default function PrivacyPage() {
  return (
    <LegalPageShell
      ...
      description={{
        ko: "사주바라가 어떤 정보를 수집하고, 왜 사용하며, 어떻게 보관하는지 설명합니다.",
        en: "This policy explains what information Sajubara collects, why it is used, and how it is stored.",
      }}
      updatedAt={UPDATED_AT}
    >
      <LegalSection title={{ ko: "1. 수집하는 개인정보", en: "1. Personal Information Collected" }}>
        <LocalizedValue
          ko="사주바라는 회원가입 및 로그인 과정에서 이메일, 소셜 로그인 식별자, 닉네임 또는 프로필 정보를 수집할 수 있습니다. 리포트 생성을 위해 이름, 생년월일, 출생시간, 성별, 양력/음력 여부, 관계, 연애·직업 상태, 저장한 사람 정보, 선택한 날짜와 목적 메모를 처리합니다. 결제 이용 시 결제 식별자, 상품명, 결제 금액, 결제 상태, 환불 처리 기록을 보관할 수 있습니다."
          en="During sign-up and login, Sajubara may collect email address, social login identifiers, nickname, or profile information. To generate reports, we process name, birth date, birth time, gender, solar/lunar calendar type, relationship, relationship and work status, saved person information, selected dates, and purpose memos. For payments, we may store payment identifiers, product name, payment amount, payment status, and refund records."
        />
      </LegalSection>
      ...
      <LegalSection title={{ ko: "3. 생성형 AI와 외부 서비스 이용", en: "3. Generative AI and External Services" }}>
        <LocalizedValue
          ko="사주바라는 리포트 본문 생성을 위해 입력 정보와 명식 데이터 일부를 생성형 AI 서비스에 전송할 수 있습니다. 인증, 데이터 저장, 배포, 결제 처리를 위해 Supabase, Vercel, OpenAI, PortOne 및 PG사 등 외부 처리 업체를 이용할 수 있으며, 처리 업체와 처리 목적이 변경되는 경우 본 방침에 반영합니다."
          en="Sajubara may send parts of input information and chart data to generative AI services to create report text. We may use external processors such as Supabase, Vercel, OpenAI, PortOne, and payment gateway providers for authentication, data storage, deployment, and payments. If processors or processing purposes change, this policy will be updated."
        />
      </LegalSection>
      ...
      <LegalSection title={{ ko: "5. 쿠키와 로컬 저장소", en: "5. Cookies and Local Storage" }}>
        <LocalizedValue
          ko="사주바라는 로그인 세션 유지, 결제 잠금 해제 상태 확인, 최근 본 사주와 보관함 표시를 위해 쿠키와 브라우저 로컬 저장소를 사용할 수 있습니다. 브라우저 설정에서 쿠키 저장을 거부할 수 있으나, 일부 기능이 제한될 수 있습니다."
          en="Sajubara may use cookies and browser local storage to maintain login sessions, check paid unlock status, and display recently viewed saju and archive data. You may refuse cookie storage in browser settings, but some features may be limited."
        />
      </LegalSection>
      ...
      <LegalSection title={{ ko: "7. 안전성 확보 조치", en: "7. Security Measures" }}>
        <LocalizedValue
          ko="사주바라는 접근 권한 관리, 인증 정보 보호, 전송 구간 암호화, 운영 로그 점검 등 개인정보 보호를 위한 기술적·관리적 조치를 적용합니다."
          en="Sajubara applies technical and administrative measures for personal information protection, including access control, protection of authentication information, encryption in transit, and operational log review."
        />
      </LegalSection>

      <LegalSection title={{ ko: "8. 개인정보 문의", en: "8. Privacy Contact" }}>
        <LocalizedValue
          ko="개인정보 관련 문의, 삭제 요청, 권리 행사는 support@sajubara.com 으로 접수합니다. 개인정보 보호책임자명, 연락처, 사업자 정보 등 운영 정보가 변경되는 경우 본 방침과 서비스 하단 고지에 반영합니다."
          en="Privacy questions, deletion requests, and rights requests can be sent to support@sajubara.com. If operating information such as the privacy officer, contact details, or business information changes, this policy and the service footer notice will be updated."
        />
      </LegalSection>
    </LegalPageShell>
  );
